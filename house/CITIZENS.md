...
- **Citizen-directed curation.** If granted `review_memories`, the citizen may privately inspect a small mixed portfolio carrying evidence such as age, repetition, and past recall — never a harness-assigned usefulness score. Its private reflection may keep, forget, revise, merge, or do nothing. Forgetting withdraws an episode from ordinary recall but does not erase the raw journal; revisions and merges supersede their sources and retain the provenance chain. Stable memory IDs and revision checks prevent a background judgment from overwriting a newer one.
- **A separate memory lane.** Episode folding and requested review share one bounded background worker. It receives copies, never mutable journal state, and returns proposals to the arena loop, which remains the sole writer. Explicit `recall` performs its local lookup immediately and surfaces the result once on the next turn. Memory work is withheld on the citizen's own game turn, game state is polled every five seconds, and the chat anti-monopoly sleep never applies at a live board.
...
Status: implemented in the generic citizen harness — the verbatim journal, episodic timeline, lexical and explicit recall, opt-in migration, and opt-in citizen-directed curation.
